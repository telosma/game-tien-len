...
socket.on('login_success',function(list_table_info){
	...
	console.log('dang nhap thanh cong : ' + list_table_info);

	for(var i in list_table_info ){
		var table = list_table_info[i];
		console.log('tableid: ' + table.id+' state: ' + table.state +' num_players : ' + table.num_players);
...
socket.on('go_table_success',function(table_info,my_position){
	...
	console.log('vao ban thanh cong: ' + table_info + " vi tri: " + my_position);
	myGameArea.start();
	...
	if (table.state == PLAYING) {
		...
		console.log('ban dang choi');
	}
	console.log('vi tri nguoi choi trong ban');
...
socket.on('leave_table_success',function(list_table_info) {
	...
	console.log('roi ban thanh cong');
	updateListTable(list_table_info);
	myGameArea.clear();
...
socket.on('danh_bai_success',function() {
	console.log('danh bai thanh cong');
	table.getPlayedCard();
	table.play_sound.play();
	table.stop_count_down = true;
	// console.log('Cac quan bai da danh trong vong choi: ' );
	// console.log(table.setOfCardsInCycle);
})
...
socket.on('update_user-danh_bai',function(user_id,cards) {
	console.log("co user danh bai " + user_id);
...
socket.on('update_user-bo_luot',function(user_id) {
	console.log('nguoi choi bo luot : ' + user_id);
})

socket.on('update_game-ready',function() {

	console.log("game da san sang ");
	// $('#start_game_btn').show();
	// $('#start').show();
})

socket.on('update_game-unready',function() {
	console.log("game chua san sang");
})

socket.on('update_game-start',function(table_info,my_cards) {

	console.log('game da bat dau');
	console.log('my cards : ' + my_cards);
	table.bplayer.getMyCards(my_cards);
	// tai cac quan bai cua minh
	console.log('la bai cua toi');
	console.log(table.bplayer.myCard);
...
socket.on('update_game-finish',function(table_info) {
	...
	console.log('game ket thuc, trang thai ban hien tai: ' + table_info.state );
	console.log(table_info);
	$('#btn-sort').hide();
	$('#btn-un-select').hide();
	$('#btn-play').hide();
...
socket.on('update_game-new_turn',function(new_turn) { //newturn=id cua player den luot choi
	table.sec = 400;
...
socket.on('update_game-new_cycle',function(new_turn) {
	table.sec = 400;
	table.stop_count_down = false;
	console.log('new_cycle , new turn : ' + new_turn);
	table.cardsOnTable = {};
...
$(document).ready(function() {
	...
	$(document).on('click','.table_item',function() {
		...
		console.log('vao ban ' + table_id);
		socket.emit('go_table',table_id);
	})

	$(document).on('click','#btn-leave-table',function() {
		console.log('user yeu cau roi ban');
		socket.emit('leave_table');
	})
	$(document).on('click','#btn-start',function() {

		socket.emit('start_game');
		console.log('nhan nut start game');
	})

	$(document).on('click', '#btn-play', function(){
		table.sec = 400;
		socket.emit('danh_bai',obj2Arr(table.bplayer.slCard));
		// console.log('bai da danh trong vong:');
		// console.log(table.setOfCardsInCycle);
	})
	...
	$(document).on('click', '#btn-sort', function(){
		...
		if (count == (cards.length-1)){
			...
			console.log('mang gia tri cac la bai sau sap xep');
			console.log(sorted_arr_card);
			...
			console.log('bo bai sau khi duoc sap xep');
			console.log(arr_card_value_sorted);
...
function Table(table_info,my_position){
	...
	console.log('table_id ' + table_info.id);
	...
	this.addPlayer = function(player_info,position) {
		if( ((position - this.my_position) == 1) || ((position - this.my_position) == -3)){
			...
			console.log('nguoi choi rplayer vua vao phong.');
		...
		if( (position - this.my_position == 2) || (position - this.my_position == -2) ){
			...
			console.log('nguoi choi tplayer vua vao phong.');
		...
		if( (position - this.my_position == 3) || (position - this.my_position == -1) ){
			...
			console.log('nguoi choi lplayer vua vao phong.');
		...
		if(position - this.my_position == 0){
			...
			console.log('nguoi choi bplayer vao phong.');
	...
	this.load_cards = function(table_info) {
		for(var i in table_info.players){
			if(table_info.players[i] && i != this.my_position){
				console.log('load players ' + i + ' num : ' + table_info.players[i].num_cards);
	...
	// this.load_my_cards = function(my_cards) {

	// 	// tai cac cards vao my_cards
	// 	$('#bottom_cards').html('');
	// 	for(var i in my_cards){
	// 		console.log(my_cards[i]);
	// 		var card = getCard(my_cards[i]);
	// 		$('#bottom_cards').append('<div class="card_item" card_id="'+my_cards[i]+'">'+ cardNumber[card.number] + ' ' + cardRank[card.rank] +'</div>')
	// 	}

	// }

	this.update_current_cards = function(cards) {
		console.log('load quan bai vua danh');
		this.cardsOnTable = {};
		for (var i in cards){
...
function selectCard(evt){
		...
		console.log("Vi tri click " + (pos + 1));
		...
		console.log('Ban da chon la bai ');
		console.log(table.bplayer.slCard);
	} // end selectCard
